...
SeedSense is an embedded computer-vision system foundation for non-destructive seed quality assessment.

The current implementation focuses on the **image acquisition and transmission pipeline**:

- An ESP32 camera captures seed images.
- The captured image is transmitted to a computer over serial communication.
- A Python application receives and reconstructs the image.
- RGB565 image data is converted to RGB888.
- The reconstructed image is saved as `seed.png`.

> **Current scope:** Machine-learning inference is not currently implemented. This repository establishes the acquisition layer for future computer-vision and machine-learning stages.
...
> `camera.cpp` and `camera.h` provide reusable camera functions for modular development. The current `SeedSense.ino` performs camera initialization and capture directly.
...
1. The image acquisition system currently uses a fixed image resolution of 160 × 120.
2. The serial port is configured manually in `capture.py`.
3. The system currently requires a computer for image reconstruction.
4. The current repository does not perform machine-learning inference.
5. Automated seed segmentation is not currently implemented.
6. Automated seed-quality classification is not currently implemented.
7. The camera GPIO configuration is specific to the configured ESP32 camera hardware.
8. Dataset files are maintained separately from the source-code repository.
...
1. Obtain compatible ESP32 camera hardware.
2. Configure the camera GPIO pins according to `SeedSense.ino`.
3. Install the ESP32 board support package in Arduino IDE.
4. Upload `SeedSense.ino`.
5. Connect the ESP32 to the host computer.
6. Identify the assigned serial port.
7. Update the `PORT` variable in `capture.py`.
8. Install the Python dependencies.
9. Run `capture.py`.
10. Verify that `seed.png` is generated.
...
SeedSense provides a hardware-software foundation for non-destructive seed image acquisition.

The current implementation demonstrates communication between an ESP32 camera and a Python host application, including custom packet framing and RGB565 image reconstruction.
